...
echo '<input type="text" class="form-control" id="inlineFormInputName2" name="searchterm" placeholder="Suchbegriff" value="' . (isset($_POST["searchterm"]) ? htmlspecialchars($_POST["searchterm"]) : "") . '">';
echo "</div>";
echo '<div class="col-md-auto mt-3 mt-md-0">';
echo '<button type="submit" class="btn btn-outline-primary ms-3"><i class="fa fa-search" aria-hidden="true"></i> Suchen</button>';
echo "</div>";
echo "</div>";
echo "</div>";
echo "</form>";
...
//no search term
    ...
    //without group
    $stmt = $libDb->prepare(
        'SELECT COUNT(*) AS number FROM base_person WHERE gruppe = "" OR gruppe IS NULL'
    );
    $stmt->execute();
    ...
    if ($anzahl > 0) {
        ...
        $stmt = $libDb->prepare(
            'SELECT * FROM base_person WHERE gruppe = "" OR gruppe IS NULL ORDER BY name'
        );
        printPersons($stmt);
    }
    ...
    //Füchse
    $stmt = $libDb->prepare(
        "SELECT COUNT(*) AS number FROM base_person WHERE gruppe = 'F'"
    );
    $stmt->execute();
    $stmt->bindColumn("number", $anzahl);
    $stmt->fetch();
    ...
    if ($anzahl > 0) {
        ...
        $stmt = $libDb->prepare(
            "SELECT * FROM base_person WHERE gruppe = 'F' ORDER BY name"
        );
        printPersons($stmt);
    }
    ...
    //Burschen
    $stmt = $libDb->prepare(
        "SELECT COUNT(*) AS number FROM base_person WHERE gruppe = 'B'"
    );
    $stmt->execute();
    $stmt->bindColumn("number", $anzahl);
    $stmt->fetch();
    ...
    if ($anzahl > 0) {
        ...
        //aktive Burschen
        $stmt = $libDb->prepare(
            "SELECT COUNT(*) AS number FROM base_person WHERE gruppe = 'B' AND (status IS NULL OR (status NOT LIKE '%ex loco%' AND status NOT LIKE '%Inaktiv%'))"
        );
        $stmt->execute();
        ...
        if ($anzahlAktiv > 0) {
            $stmt = $libDb->prepare(
                "SELECT * FROM base_person WHERE gruppe = 'B' AND (status IS NULL OR (status NOT LIKE '%ex loco%' AND status NOT LIKE '%Inaktiv%')) ORDER BY name"
            );
            printPersons($stmt);
        }
        ...
        //inaktive Burschen
        $stmt = $libDb->prepare(
            "SELECT COUNT(*) AS number FROM base_person WHERE gruppe = 'B' AND (status LIKE '%ex loco%' OR status LIKE '%Inaktiv%')"
        );
        $stmt->execute();
        ...
        if ($anzahlInaktivExLoco > 0) {
            $stmt = $libDb->prepare(
                "SELECT * FROM base_person WHERE gruppe = 'B' AND (status LIKE '%ex loco%' OR status LIKE '%Inaktiv%') ORDER BY name"
            );
            printPersons($stmt);
        }
    }
    ...
    //Philister
    $stmt = $libDb->prepare(
        "SELECT COUNT(*) AS number FROM base_person WHERE gruppe = 'P'"
    );
    $stmt->execute();
    $stmt->bindColumn("number", $anzahl);
    $stmt->fetch();
    ...
    if ($anzahl > 0) {
        ...
        $stmt = $libDb->prepare(
            "SELECT * FROM base_person WHERE gruppe = 'P' ORDER BY name"
        );
        printPersons($stmt);
    }
    ...
    if ($libGenericStorage->loadValueInCurrentModule("show_group_y")) {
        //Vereinsfreunde
        $stmt = $libDb->prepare(
            "SELECT COUNT(*) AS number FROM base_person WHERE gruppe = 'Y'"
        );
        $stmt->execute();
        $stmt->bindColumn("number", $anzahl);
        $stmt->fetch();
        ...
        if ($anzahl > 0) {
            ...
            $stmt = $libDb->prepare(
                "SELECT * FROM base_person WHERE gruppe = 'Y' ORDER BY name"
            );
            printPersons($stmt);
        }
    }
