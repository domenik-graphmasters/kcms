...
if (
    isset($_POST["registrierung_name"]) ||
    isset($_POST["registrierung_telnr"]) ||
    isset($_POST["registrierung_mail"]) ||
    isset($_POST["registrierung_geburtsdatum"]) ||
    isset($_POST["registrierung_pwd1"]) ||
    isset($_POST["registrierung_pwd2"])
) {
    $formSent = true;

    if (
        !isset($_POST["registrierung_name"]) ||
        $_POST["registrierung_name"] == ""
    ) {
    ...
    if (
        !isset($_POST["registrierung_emailadresse"]) ||
        $_POST["registrierung_emailadresse"] == ""
    ) {
        $libGlobal->errorTexts[] = "Bitte geben Sie eine E-Mail-Adresse an.";
        $formError = true;
    } elseif (
        isset($_POST["registrierung_emailadresse"]) &&
        !$libString->isValidEmail($_POST["registrierung_emailadresse"])
    ) {
    ...
    if (
        ...
        $libGlobal->errorTexts[] =
            "Das Passwort ist nicht komplex genug. " .
            $libAuth->getPasswordRequirements();
        $formError = true;
    } else {
        if (
            !isset($_POST["registrierung_pwd2"]) ||
            trim($_POST["registrierung_pwd2"]) == ""
        ) {
            $libGlobal->errorTexts[] =
                "Bitte geben Sie das Passwort ein zweites Mal ein.";
            $formError = true;
        } else {
...
if ($formSent && !$formError) {
    ...
    $text =
        "Auf " .
        $libGlobal->getSiteUrl() .
        " wurde folgende Registrierungsanfrage für das Intranet gestellt: " .
        PHP_EOL;
    $text .= PHP_EOL;
    $text .=
        "Name: " .
        $libString->protectXSS($_POST["registrierung_name"]) .
        PHP_EOL;
    $text .=
        "E-Mail-Adresse: " .
        $libString->protectXSS(
            strtolower($_POST["registrierung_emailadresse"])
        ) .
        PHP_EOL;
    $text .=
        "Telefonnummer: " .
        $libString->protectXSS($_POST["registrierung_telnr"]) .
        PHP_EOL;
    $text .=
        "Geburtsdatum: " .
        $libString->protectXSS($_POST["registrierung_geburtsdatum"]) .
        PHP_EOL;
    $text .= "Passwort-Hash: " . $password_hash . PHP_EOL;
    $text .= PHP_EOL;
    $text .=
        "Die Freischaltung für das Intranet erfolgt, indem der Internetwart die Daten nach einer Plausibilitätsprüfung im Personenprofil speichert." .
        PHP_EOL;
    ...
    $text .= PHP_EOL;
    $text .=
        "Du wurdest mit der E-Mail-Adresse " .
        $libString->protectXSS($_POST["registrierung_emailadresse"]) .
    ...
    $stmt = $libDb->prepare(
        "SELECT internetwart FROM base_semester ORDER BY SUBSTRING(semester, 3) DESC LIMIT 1"
    );
    $stmt->execute();
    ...
    $stmt = $libDb->prepare(
        "SELECT email FROM base_person WHERE id=:internetwartId"
    );
    ...
    $mail->Subject =
        "[" . $libConfig->verbindungName . "] Intranet-Registrierung";
    $mail->Body = $text;
    ...
    if ($mailsent) {
        ...
        echo '<p class="mb-4">Die Nachricht konnte nicht verschickt werden. Bitte schreiben Sie direkt an die E-Mail-Adresse ' .
            $libConfig->emailWebmaster .
            "</p>";
    }
} else {
    ...
    if ($libGlobal->getSiteUrlAuthority() != "") {
        $sslProxyUrl = $libGenericStorage->loadValueInCurrentModule(
            "ssl_proxy_url"
        );
    ...
    echo '<form method="post" action="' .
        $urlPrefix .
        'index.php?pid=registration" class="">';
    echo "<fieldset>";

    $libForm->printTextInput(
        "registrierung_name",
        "Vorname und Nachname",
        $libString->protectXSS($registrierung_name),
        "text",
        false,
        true
    );
    $libForm->printTextInput(
        "registrierung_telnr",
        "Telefonnummer",
        $libString->protectXSS($registrierung_telnr),
        "tel",
        false,
        true
    );
    $libForm->printTextInput(
        "registrierung_emailadresse",
        "E-Mail-Adresse",
        $libString->protectXSS($registrierung_emailadresse),
        "email",
        false,
        true
    );
    $libForm->printTextInput(
        "registrierung_geburtsdatum",
        "Geburtsdatum",
        $libString->protectXSS($registrierung_geburtsdatum),
        "date",
        false,
        true
    );
    ...
    $libForm->printSubmitButton(
        '<i class="fa fa-pencil-square-o" aria-hidden="true"></i> Abschicken'
    );
