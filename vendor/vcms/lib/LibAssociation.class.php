...
class LibAssociation
{
    ...
    function getVereinNameString(string $associationId): string
    {
        global $libDb;

        $stmt = $libDb->prepare(
            "SELECT titel, name FROM base_verein WHERE id = :id"
        );
    ...
    function getToechterString($associationId, $pid)
    {
        ...
        $stmt = $libDb->prepare(
            "SELECT tochter.id, tochter.titel, tochter.name FROM base_verein AS mutter, base_verein AS tochter WHERE mutter.id = tochter.mutterverein AND mutter.id = :id"
        );
        $stmt->bindValue(":id", $associationId, PDO::PARAM_INT);
        $stmt->execute();
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($pid != "") {
                $retstr .=
                    '<a href="index.php?pid=verein&amp;id=' . $row["id"] . '">';
            }
    ...
    function getFusioniertString($associationId, $pid)
    {
        ...
        $stmt = $libDb->prepare(
            "SELECT fusionierend.id, fusionierend.titel, fusionierend.name FROM base_verein AS fusionierend, base_verein AS fusioniert WHERE fusioniert.id = fusionierend.fusioniertin AND fusioniert.id = :id"
        );
        $stmt->bindValue(":id", $associationId, PDO::PARAM_INT);
        $stmt->execute();
    ...
    function getAnsprechbarerAktivenVorstandIds()
    {
        ...
        $stmt = $libDb->prepare(
            "SELECT senior, jubelsenior, consenior, fuchsmajor, fuchsmajor2, scriptor, quaestor FROM base_semester WHERE semester = :semester"
        );
    ...
    function getValideInternetWarte()
    {
        ...
        $stmt = $libDb->prepare(
            'SELECT COUNT(*) AS anzahlsemester, base_person.id FROM base_person, base_semester WHERE base_semester.internetwart = base_person.id AND gruppe != "X" AND gruppe != "T" AND gruppe != "C" AND gruppe != "W" AND gruppe != "G" AND email IS NOT NULL AND email != "" AND password_hash IS NOT NULL AND password_hash != "" GROUP BY id'
        );
        $stmt->execute();
    ...
    function importAssociations()
    {
        ...
        if (empty($json)) {
            $libGlobal->errorTexts[] =
                "Die Vereinsdaten konnten nicht von " .
                $jsonUrl .
                " geladen werden.";
        } else {
            if (!is_array($json)) {
            ...
            if (!is_array($json)) {
                $libGlobal->errorTexts[] =
                    "Die Vereinsdaten können nicht verarbeitet werden.";
            } else {
                $libGlobal->notificationTexts[] =
                    "Importiere " . count($json) . " Vereins-Datensätze.";
    ...
    function isValidKvAssociation($association)
    {
        $result =
            ...
            is_numeric($association["id"]) &&
            isset($association["name"]) &&
            !empty($association["name"]) &&
            isset($association["dachverbandnr"]) &&
            is_numeric($association["dachverbandnr"]);
        return $result;
    }
    ...
    function importAssociation($association)
    {
        ...
        $stmt = $libDb->prepare(
            "SELECT COUNT(*) as number FROM base_verein WHERE name = :name AND dachverbandnr = :dachverbandnr"
        );
        $stmt->bindValue(":name", $association["name"]);
        $stmt->bindValue(
            ":dachverbandnr",
            $association["dachverbandnr"],
            PDO::PARAM_INT
        );
        $stmt->execute();
        ...
        if ($number > 0) {
            $stmt = $libDb->prepare(
                "UPDATE base_verein SET kuerzel=:kuerzel, aktivitas=:aktivitas, ahahschaft=:ahahschaft, titel=:titel, rang=:rang, dachverband=:dachverband, zusatz1=:zusatz1, strasse1=:strasse1, ort1=:ort1, plz1=:plz1, land1=:land1, telefon1=:telefon1, datum_gruendung=:datum_gruendung, webseite=:webseite, wahlspruch=:wahlspruch, farbenstrophe=:farbenstrophe, fuchsenstrophe=:fuchsenstrophe, bundeslied=:bundeslied, farbe1=:farbe1, farbe2=:farbe2, farbe3=:farbe3, farbe4=:farbe4 WHERE name=:name AND dachverbandnr=:dachverbandnr"
            );

            $stmt->bindValue(
                ":kuerzel",
                $libString->protectXss($association["kuerzel"])
            );
            $stmt->bindValue(
                ":aktivitas",
                $libString->protectXss($association["aktivitas"]),
                PDO::PARAM_INT
            );
            $stmt->bindValue(
                ":ahahschaft",
                $libString->protectXss($association["ahahschaft"]),
                PDO::PARAM_INT
            );
            $stmt->bindValue(
                ":titel",
                $libString->protectXss($association["titel"])
            );
            $stmt->bindValue(
                ":rang",
                $libString->protectXss($association["rang"])
            );
            $stmt->bindValue(
                ":dachverband",
                $libString->protectXss($association["dachverband"])
            );
            $stmt->bindValue(
                ":zusatz1",
                $libString->protectXss($association["zusatz1"])
            );
            $stmt->bindValue(
                ":strasse1",
                $libString->protectXss($association["strasse1"])
            );
            $stmt->bindValue(
                ":ort1",
                $libString->protectXss($association["ort1"])
            );
            $stmt->bindValue(
                ":plz1",
                $libString->protectXss($association["plz1"])
            );
            $stmt->bindValue(
                ":land1",
                $libString->protectXss($association["land1"])
            );
            $stmt->bindValue(
                ":telefon1",
                $libString->protectXss($association["telefon1"])
            );
            $stmt->bindValue(
                ":datum_gruendung",
                $libString->protectXss($association["datum_gruendung"])
            );
            $stmt->bindValue(
                ":webseite",
                $libString->protectXss($association["webseite"])
            );
            $stmt->bindValue(
                ":wahlspruch",
                $libString->protectXss($association["wahlspruch"])
            );
            $stmt->bindValue(
                ":farbenstrophe",
                $libString->protectXss($association["farbenstrophe"])
            );
            $stmt->bindValue(
                ":fuchsenstrophe",
                $libString->protectXss($association["fuchsenstrophe"])
            );
            $stmt->bindValue(
                ":bundeslied",
                $libString->protectXss($association["bundeslied"])
            );
            $stmt->bindValue(
                ":farbe1",
                $libString->protectXss($association["farbe1"])
            );
            $stmt->bindValue(
                ":farbe2",
                $libString->protectXss($association["farbe2"])
            );
            $stmt->bindValue(
                ":farbe3",
                $libString->protectXss($association["farbe3"])
            );
            $stmt->bindValue(
                ":farbe4",
                $libString->protectXss($association["farbe4"])
            );
            ...
            $stmt->bindValue(
                ":dachverbandnr",
                $libString->protectXss($association["dachverbandnr"]),
                PDO::PARAM_INT
            );
            $stmt->bindValue(
                ":zusatz1",
